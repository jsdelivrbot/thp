...
def get_all_crypto_names_and_trading_values 
	begin
		...
		xpath = '//a[@class="currency-name-container"]'
...
def get_crypto_names_and_trading_values_every_y_seconds (y)
	...
	(0..99).each { |i|
		puts "LANCEMENT DE LA REQUETE: #{Time.now}"
		puts "---------------------------------------------------"
		wallet_value_calculator($x, 1.177)
		puts " "
		sleep(y);
	}
end
...
def wallet_value_calculator (array, dolls)
	...
	puts "Your wallet value totals " + (total_wallet_value/dolls.to_f).to_s + " euros"
end
...
$x = [
	...
	{:crypto_name =>"KickCoin",:number_crypto =>1200.0}
]
...
get_crypto_names_and_trading_values_every_y_seconds(120)
=begin
